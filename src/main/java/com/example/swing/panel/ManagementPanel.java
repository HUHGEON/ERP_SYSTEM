...
import com.example.swing.dialog.ManagementDialog;
...
public class ManagementPanel extends JPanel {
    ...
    private static final String[] COLUMNS = {"ID", "직원명", "권한단계"};

    private final ManagementDAO dao = new ManagementDAO();
    private final DefaultTableModel tableModel = new DefaultTableModel(COLUMNS, 0) {
        @Override public boolean isCellEditable(int r, int c) { return false; }
    };
    private final JTable table = new JTable(tableModel);
    private final JTextField nameField = new JTextField(15);
    private List<Management> currentList;
    ...
    public ManagementPanel() {
        ...
        table.getColumnModel().getColumn(2).setPreferredWidth(70);   // 권한단계
    ...
    private void loadData() {
        try {
            ...
            for (Management m : currentList) tableModel.addRow(new Object[]{m.getId(), m.getEmployeeName(), m.getPermissionLevel()});
        } catch (Exception ex) { JOptionPane.showMessageDialog(this, "오류: " + ex.getMessage(), "DB 오류", JOptionPane.ERROR_MESSAGE); }
    }

    private void openDialog(Management mgmt) {
        JFrame frame = (JFrame) SwingUtilities.getWindowAncestor(this);
        ManagementDialog dialog = new ManagementDialog(frame, mgmt, dao);
    ...
    private void deleteSelected() {
        ...
        Management m = currentList.get(table.convertRowIndexToModel(row));
        if (JOptionPane.showConfirmDialog(this, "'" + m.getEmployeeName() + "' 경영관리 정보를 삭제하시겠습니까?",
                "삭제 확인", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            try { dao.delete(m.getId()); loadData(); }
