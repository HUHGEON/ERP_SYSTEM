...
public class PmEvaluationDialog extends JDialog {
    ...
    private final PmEvaluation editing;
    ...
    private void loadExisting() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            selectInCombo(pmBox, d -> d.getId() == editing.getPmId());
    ...
    private void buildUI() {
        ...
        lc.gridy = 2; fc.gridy = 2; form.add(new JLabel("PM:"),            lc); form.add(pmBox,            fc);
    ...
    private void save() {
        ...
        if (pp == null || pm == null || category == null) {
            JOptionPane.showMessageDialog(this, "프로젝트 참여, 카테고리, PM을 모두 선택하세요.");
            return;
        }
    ...
    private void insertChild(Connection c, int id, int pmId) throws Exception {
        try (PreparedStatement ps = c.prepareStatement(
            "INSERT INTO pm_evaluation(id,pm_id) VALUES(?,?)")) {
            ps.setInt(1,id); ps.setInt(2,pmId); ps.executeUpdate();
        }
    }
    private void updateChild(Connection c, int id, int pmId) throws Exception {
        try (PreparedStatement ps = c.prepareStatement(
            "UPDATE pm_evaluation SET pm_id=? WHERE id=?")) {
            ps.setInt(1,pmId); ps.setInt(2,id); ps.executeUpdate();
        }
    }
