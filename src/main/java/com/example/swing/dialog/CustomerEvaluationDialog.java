...
public class CustomerEvaluationDialog extends JDialog {
    ...
    private final CustomerEvaluation editing;
    ...
    private void loadCombos() {
        try {
            ...
            for (Customer c : new ProjectDAO().getAllCustomers())
    ...
    private void loadExisting() {
        String sql = "SELECT ev.participation_id, ev.participation_category " +
                     "FROM evaluation ev WHERE ev.id = ?";
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    selectInCombo(participationBox, pp -> pp.getId() == participationId);
                    categoryBox.setSelectedItem(cat);
                }
            }
            selectInCombo(customerBox, c -> c.getId() == editing.getCustomerId());

            String itemSql = "SELECT rate, content FROM evaluation_item WHERE evaluation_id = ? ORDER BY id";
    ...
    private void buildUI() {
        ...
        lc.gridy = 2; fc.gridy = 2; form.add(new JLabel("발주처:"),        lc); form.add(customerBox,      fc);
    ...
    private void save() {
        ...
        if (pp == null || customer == null || category == null) {
            JOptionPane.showMessageDialog(this, "프로젝트 참여, 카테고리, 발주처를 모두 선택하세요.");
            return;
        }
        ...
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            try {
                ...
                if (editing == null) {
                    ...
                    insertCustomerEvaluation(conn, evalId, customer.getId());
                } else {
                    evalId = editing.getId();
                    updateEvaluation(conn, evalId, pp.getId(), category);
                    updateCustomerEvaluation(conn, evalId, customer.getId());
    ...
    private void insertCustomerEvaluation(Connection c, int id, int customerId) throws Exception {
        try (PreparedStatement ps = c.prepareStatement(
            "INSERT INTO customer_evaluation(id,customer_id) VALUES(?,?)")) {
            ps.setInt(1,id); ps.setInt(2,customerId); ps.executeUpdate();
        }
    }
    private void updateCustomerEvaluation(Connection c, int id, int customerId) throws Exception {
        try (PreparedStatement ps = c.prepareStatement(
            "UPDATE customer_evaluation SET customer_id=? WHERE id=?")) {
            ps.setInt(1,customerId); ps.setInt(2,id); ps.executeUpdate();
        }
    }
